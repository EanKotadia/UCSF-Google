...
-- Members table (Navigators, Charge d'Affaires, Directors, EB, OC)
CREATE TABLE members (
  ...
  category TEXT NOT NULL, -- 'Navigator', 'Charge d''Affaires', 'Director', 'EB', 'OC'
  sort_order INTEGER DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE rankings (
  ...
  award TEXT NOT NULL, -- e.g. Best Delegate
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE sponsors (
  ...
  name TEXT NOT NULL,
  logo_url TEXT,
  tier TEXT, -- 'Platinum', 'Gold', etc.
  website_url TEXT,
  sort_order INTEGER DEFAULT 0,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Schedule table (Event Flow)
